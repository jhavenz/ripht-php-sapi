...
Or install to one of the default fallback locations: `~/.ripht/php`, `~/.local/php`, or `/usr/local`. The recommended local shared prefix is `~/.ripht/php`.
    
> Important Notes:
> This crate is focuses on the non-ZTS build of PHP. 
> There aren't currently plans to support ZTS builds.
>
> Tip: Tools like [Static PHP CLI](https://github.com/crazywhalecc/static-php-cli) can simplify building PHP with the embed SAPI. If the prefix includes `lib/bia-link-flags.txt`, Ripht uses that generated linker manifest. Without it, Ripht falls back to best-effort static dependency scanning from the prefix `lib/` directory. See [CONTRIBUTING.md](CONTRIBUTING.md) for development setup options.
...
- The build script expects a PHP build root that contains `lib/libphp.a` (static embed SAPI) and headers. Install it at `~/.ripht/php` or set `RIPHT_PHP_SAPI_PREFIX` to point at your PHP build prefix.
- Prefixes generated by Bia/static-php-cli should include `lib/bia-link-flags.txt`; Ripht consumes that manifest when present and otherwise uses best-effort static dependency scanning.
